...
class VoxelRCNNHead(RoIHeadTemplate):
    ...
    def roi_grid_pool(self, batch_dict):
        ...
        # compute the voxel coordinates of grid points: Convert grid points to voxel coordinates
        ...
        batch_idx = rois.new_zeros(batch_size, roi_grid_coords.shape[1], 1) # (B, N*6*6*6, 1)
        # Each ROI grid point needs to know which batch it belongs to.
        ...
        # Record the number of ROI grid points for each batch.
        roi_grid_batch_cnt = rois.new_zeros(batch_size).int().fill_(roi_grid_coords.shape[1])

        # Multi-scale voxel pooling
        pooled_features_list = []
        # Traversing multi-scale 3D feature volumes
        for k, src_name in enumerate(self.pool_cfg.FEATURES_SOURCE):
            pool_layer = self.roi_grid_pool_layers[k]
            cur_stride = batch_dict['multi_scale_3d_strides'][src_name]
            cur_sp_tensors = batch_dict['multi_scale_3d_features'][src_name] # Sparse voxel tensor of a certain scale
            ...
            # Convert voxel discrete index to voxel center coordinates in world coordinates.
            cur_coords = cur_sp_tensors.indices # (bs_id, z, y, x)
            ...
            # Count the number of voxels in each batch
            cur_voxel_xyz_batch_cnt = cur_voxel_xyz.new_zeros(batch_size).int()
            for bs_idx in range(batch_size):
                cur_voxel_xyz_batch_cnt[bs_idx] = (cur_coords[:, 0] == bs_idx).sum()
            # get voxel2point tensor: (B, Z, Y, X) This dense tensor is associated with voxel.
            ...
            pooled_features = pool_layer(
                xyz=cur_voxel_xyz.contiguous(), # Voxel feature's spatial coordinates (center point coordinates)
                xyz_batch_cnt=cur_voxel_xyz_batch_cnt, # number of voxels in each batch
                new_xyz=roi_grid_xyz.contiguous().view(-1, 3), # RoI grid point center coordinates (query points)
                new_xyz_batch_cnt=roi_grid_batch_cnt, # number of roi grid in each batch
                new_coords=cur_roi_grid_coords.contiguous().view(-1, 4), # roi grid voxel coordinates
                features=cur_sp_tensors.features.contiguous(),
                voxel2point_indices=v2p_ind_tensor
            )

            pooled_features = pooled_features.view(-1, self.pool_cfg.GRID_SIZE ** 3, pooled_features.shape[-1]) # (BxN, 6x6x6, C)
            pooled_features_list.append(pooled_features)
        # Features aggregated from the neighborhood of roi grid points on multi-scale feature maps
    ...
    def get_global_grid_points_of_roi(self, rois, grid_size):
        """
        Transform the RoI internal point coordinates to the global point cloud coordinate system.
        """
        rois = rois.view(-1, rois.shape[-1]) # (BxN, 7 + C)
        batch_size_rcnn = rois.shape[0]

        local_roi_grid_points = self.get_dense_grid_points(rois, batch_size_rcnn, grid_size)  # (B, 6x6x6, 3)
        # The coordinate axis rotates from ROI to global
        global_roi_grid_points = common_utils.rotate_points_along_z(
            local_roi_grid_points.clone(), rois[:, 6]
        ).squeeze(dim=1)
        global_center = rois[:, 0:3].clone()
        # After adding the center point coordinates of ROI in the global coordinate system, \
        # the local point coordinates of ROI become global point coordinates.
    ...
    def forward(self, batch_dict):
        ...
        # Box Refinement
        pooled_features = pooled_features.view(pooled_features.size(0), -1) # (B*N, 6*6*6*C) flatten ROI grid feature
        shared_features = self.shared_fc_layer(pooled_features) # (B*N, hidden_dims)
        rcnn_cls = self.cls_pred_layer(self.cls_fc_layers(shared_features)) # (B*N, num_class)
        rcnn_reg = self.reg_pred_layer(self.reg_fc_layers(shared_features)) # (B*N, code_size*num_class)
        ...
        if not self.training:
            # Combine the bbox offset from the network output with the ROI to obtain the final predicted box.
